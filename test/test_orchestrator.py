...
"""
    ...
    def test_run_with_mocked_dependencies(
        ...
        with (
            patch.object(orchestrator, "load_context", return_value=("", "")),
            ...
            patch("pathlib.Path.exists", return_value=False),
            patch.object(
                orchestrator.script_gen,
                "generate_visual_prompt",
                return_value="Visual prompt",
            ),
            patch.object(
                orchestrator.scene_concept_gen,
                "ensure_scene_concept_image",
                return_value=None,
            ),
        ):
    ...
    @patch("magicplay.core.orchestrator.MediaUtils")
    @patch("magicplay.core.orchestrator.ScriptAnalyzer")
    def test_run_with_predefined_scenes(
        self,
        mock_script_analyzer_class,
        mock_media_utils,
        mock_video_gen_class,
        mock_script_gen_class,
        mock_get_scenes,
        tmp_path,
    ):
    ...
    def test_run_exception_handling(self):
        ...
        with (
            ...
            patch.object(orchestrator, "_ensure_character_images", return_value=None),
            patch(
                "magicplay.core.orchestrator.DataManager.get_scenes_prompts",
                return_value=[],
            ),
            patch("builtins.print"),
        ):
...
class TestStoryOrchestrator:
    ...
    def test_run_with_episodes(self, mock_stitch, mock_orchestrator_class, mock_get_episodes, tmp_path):
        ...
        mock_orchestrator_class.assert_any_call(
