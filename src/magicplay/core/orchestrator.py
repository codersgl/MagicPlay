        ...
        try:
            ...
            if not has_characters:
                ...
                if anchors_dir.exists():
                    existing_files = list(anchors_dir.glob("*.png")) + list(anchors_dir.glob("*.jpg")) + list(anchors_dir.glob("*.jpeg"))
                    if existing_files:
            ...
            if anchors_dir.exists():
                ...
                if existing_files:
                    ...
                    if loaded_images:
                        print(f"Associated {len(loaded_images)} existing images with characters: {list(loaded_images.keys())}")
    ...
    def run(self, initial_memory: str = "") -> Tuple[Optional[Path], str]:
        ...
        for scene_name, prompt_file in scenes_to_process:
            ...
            if not script_path.exists():
                ...
                # Load character profiles for Visual Tags anchoring
                character_profiles = {}
                try:
                    consistency_manager = StoryConsistencyManager(self.story_name)
                    consistency_manager.load_from_story_bible(story_ctx)
                    character_profiles = consistency_manager.get_all_formatted_visual_tags()
                    if character_profiles:
                        print(f"Loaded {len(character_profiles)} character profiles for Visual Tags anchoring")
                except Exception as e:
                    print(f"Warning: Failed to load character profiles: {e}")
            ...
            if script_path.exists():
                ...
                # Physics check for newly generated scripts
                if not script_path.exists() or prompt_file is None:
                    try:
            ...
            if not video_path.exists():
                ...
                try:
                    # Summarize script into visual prompt (with character profiles for consistency)
                    visual_prompt_text = self.script_gen.generate_visual_prompt(
                        script_path,
                        character_profiles=character_profiles if 'character_profiles' in locals() else None,
                    )

                    # Phase 2: Generate scene concept image (first frame)
                    scene_concept_image = None
                    try:
                        # Load story context for character images
                        character_images = {}
                        try:
                            ...
                            print(
                                f"Warning: Failed to load character images for consistency: {ce}"
                            )
                    ...
                    if analysis_result:
                        ...
                        print(
                            ...
                            f"estimated_duration={duration}s, "
                            f"complexity={analysis_result.complexity_score:.2f}"
                        )

                    # Phase 3: Multi-frame generation option
                    use_multi_frame = True  # Enable multi-frame generation
                    if use_multi_frame and duration and duration > 8:
                        # For longer scenes, try multi-frame generation
                        print(f"Using multi-frame generation for {scene_name} (duration: {duration}s)...")
                        try:
                            ...
                            if segments and len(segments) > 1:
                                ...
                                if stitched_video_path and Path(stitched_video_path).exists():
                                    ...
                                    print(f"Multi-frame stitching failed, falling back to single segment")
                                ...
                                print(f"Multi-frame generation returned {len(segments) if segments else 0} segments, falling back to single segment")
                                # Fallback to single video generation
                                video_path = self._generate_single_video(
                                    visual_prompt_text,
                                    video_path,
